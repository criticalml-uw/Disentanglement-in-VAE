...
class VAE(nn.Module):
    def __init__(self, z_dim, use_cuda=False, prior_dist=dist.Normal(), q_dist=dist.Normal(), x_dist = dist.Bernoulli(),
                 include_mutinfo=True, tcvae=False, conv=False, mss=False, mnist = False, d3 = False, batch_size = 32):
...
def main():
    ...
    parser.add_argument('--save', default='../Results/beta_tcvae/Random_Exp')
    ...
    vae = VAE(z_dim=args.latent_dim, use_cuda=True, prior_dist=prior_dist, q_dist=q_dist, x_dist = x_dist,
        include_mutinfo=not args.exclude_mutinfo, tcvae=args.tcvae, conv=args.conv, mss=args.mss, mnist = mnist, d3 = d3, batch_size=args.batch_size)
