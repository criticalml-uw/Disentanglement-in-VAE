...
class Normal(nn.Module):
    ...
    def _check_inputs(self, size, mu_logsigma):
        if size is None and mu_logsigma is None:
            ...
            '''print("MU_LOGSIGMA", mu_logsigma.shape)'''
            mu = mu_logsigma.select(-1, 0)
            logsigma = mu_logsigma.select(-1, 1)
            '''print("MU AND LOGSIMA", mu.shape, logsigma.shape)'''
            return mu, logsigma
        else:
            raise ValueError(
    ...
    def reparametrize(self, mu, logvar):
        ...
        eps_1 = std.data.new(std.size()).normal_()
        return eps_1.mul(std).add_(mu)
    ...
    def sample(self, size=None, params=None):
        ...
        '''sample = self.reparametrize(mu, logsigma)'''

        return sample

    def sample_return_everything(self, size=None, params=None):
        mu, logsigma = self._check_inputs(size, params)
        #print("MU inside Dist Lib", torch.max(mu))
        std_z = Variable((torch.randn(mu.size())).type_as(mu.data))
        sample = std_z * torch.exp(logsigma) + mu
        return sample, std_z, logsigma, mu
