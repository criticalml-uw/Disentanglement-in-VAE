...
def plot_vs_gt_shapes(vae, shapes_dataset, save, z_inds=None):
    ...
    qz_params = qz_params.view(3, 6, 40, 32, 32, K, nparams) #(shape, scale, orientation (rotation), posx, posy, z_dim, 2(for norm))

    # z_j is inactive if Var_x(E[z_j|x]) < eps.
    qz_means = qz_params[:, :, :, :, :, :, 0]        #(shape, scale, orientation (rotation), posx, posy, z_dim, 1(only mean))
