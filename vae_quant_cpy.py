...
class d3ConvEncoder(nn.Module):
    def __init__(self, output_dim):
        ...
        self.conv00 = nn.Conv2d(3,32, 5, 1, 2)
        self.bn0 = nn.BatchNorm2d(32)
        self.conv1 = nn.Conv2d(32, 32, 4, 2, 1)  # 32 x 32
        self.conv11 = nn.Conv2d(32, 32, 5, 1, 2)
    ...
    def forward(self, x):
        h = x.view(-1, 3, 64, 64)
        h = self.act(self.bn0(self.conv00(h)))
        h = self.act(self.bn1(self.conv1(h)))
        h = self.act(self.bn1(self.conv11(h)))
        h = self.act(self.bn1(self.conv11(h)))
...
class d3ConvDecoder(nn.Module):
    def __init__(self, input_dim):
        ...
        self.conv21 = nn.ConvTranspose2d(64, 64, 1, 1, 0)  # 4 * 4
        self.conv22 = nn.ConvTranspose2d(64, 64, 3, 1, 1)  #4 * 4
        self.conv23 = nn.ConvTranspose2d(64, 64, 5, 1, 2)  # 4 * 4
        self.conv24 = nn.ConvTranspose2d(64, 64, 2, 2, 2)  # 4 * 4
        self.bn2 = nn.BatchNorm2d(64)
        self.conv3 = nn.ConvTranspose2d(64, 64, 4, 2, 1)  # 8 x 8
        self.conv31 = nn.ConvTranspose2d(64, 64, 1, 1, 0)  # 8 x 8
        self.conv32 = nn.ConvTranspose2d(64, 64, 3, 1, 1)  # 8 x 8
        self.conv33 = nn.ConvTranspose2d(64, 64, 5, 1, 2)  # 8 x 8
        self.conv33 = nn.ConvTranspose2d(64, 64, 2, 2, 4)  # 8 x 8
        self.bn3 = nn.BatchNorm2d(64)
        self.conv4 = nn.ConvTranspose2d(64, 32, 4, 2, 1)  # 16 x 16
        self.conv41 = nn.ConvTranspose2d(32, 32, 1, 1, 0)  # 16 x 16
        self.conv42 = nn.ConvTranspose2d(32, 32, 3, 1, 1)  # 16 x 16
        self.conv43 = nn.ConvTranspose2d(32, 32, 5, 1, 2)  # 16 x 16
        self.bn4 = nn.BatchNorm2d(32)
        self.conv5 = nn.ConvTranspose2d(32, 32, 4, 2, 1)  # 32 x 32
        self.conv51 = nn.ConvTranspose2d(32, 32, 1, 1, 0)  # 32 x 32
        self.conv52 = nn.ConvTranspose2d(32, 32, 3, 1, 1)  # 32 x 32
        self.conv53 = nn.ConvTranspose2d(32, 32, 5, 1, 2)  # 32 x 32
        self.bn5 = nn.BatchNorm2d(32)
        self.conv_final = nn.ConvTranspose2d(32, 3, 4, 2, 1)
        self.conv_final1 = nn.ConvTranspose2d(32, 32, 1, 1, 0)  # 32 x 32
        self.conv_final2 = nn.ConvTranspose2d(32, 32, 3, 1, 1)  # 32 x 32
        self.conv_final3 = nn.ConvTranspose2d(32, 32, 5, 1, 2)  # 32 x 32
    ...
    def forward(self, z):
        ...
        h = self.act(self.bn1(self.conv1(h)))
        h = self.act(self.bn2(self.conv2(h)))
        h = self.act(self.bn2(self.conv21(h)))
        h = self.act(self.bn2(self.conv22(h)))
        h = self.act(self.bn2(self.conv23(h)))
        h = self.act(self.bn3(self.conv3(h)))
        h = self.act(self.bn3(self.conv31(h)))
        h = self.act(self.bn3(self.conv32(h)))
        h = self.act(self.bn3(self.conv33(h)))
        h = self.act(self.bn4(self.conv4(h)))
        h = self.act(self.bn4(self.conv41(h)))
        h = self.act(self.bn4(self.conv42(h)))
        h = self.act(self.bn4(self.conv43(h)))
        h = self.act(self.bn5(self.conv5(h)))
        h = self.act(self.bn5(self.conv51(h)))
        h = self.act(self.bn5(self.conv52(h)))
        h = self.act(self.bn5(self.conv53(h)))
        '''h = self.act(self.bn1(self.conv51(h)))
        h = self.act(self.bn1(self.conv52(h)))
        h = self.act(self.bn1(self.conv53(h)))'''
...
class VAE(nn.Module):
    def __init__(self, z_dim, use_cuda=False, prior_dist=dist.Normal(), q_dist=dist.Normal(),
                 include_mutinfo=True, tcvae=False, conv=False, mss=False, mnist = False, d3 = False):
...
def main():
    ...
    parser.add_argument('-d', '--dataset', default='shapes', type=str, help='dataset name',
        choices=['shapes', 'faces', 'mnist', '3dshapes'])
    ...
    parser.add_argument('--save', default='../Results/beta_tcvae/dSprites_Retrain')
